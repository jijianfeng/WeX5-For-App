define(function(require){
	...
	Model.prototype.postClick = function(event){
		...
		var id = context.getRequestParameter("id");
		//alert(id);
		justep.Shell.showPage(require.toUrl("./post.w?id="+id));
	};
	...
	Model.prototype.detailCustomRefresh = function(event){
        ...
        $.ajax({
            type: "POST",
            url: require.toUrl('http://42.96.159.122:8080/WeX5/mobile/text_detail?id='+id),
            dataType: 'json',
            async: false,
            cache: false,
            success: function(data){
	...
	Model.prototype.span8Click = function(event){
		...
		$.ajax({
            type: "POST",
            url: require.toUrl('http://42.96.159.122:8080/WeX5/mobile/comment_add?id='+id+"&content="+content+"&type=2&userId="+justep.Shell.userId.get()+"&userType="+localStorage.getItem("userType")+"&listNumber=2"),
            dataType: 'json',
            async: false,
            cache: false,
            success: function(data){
